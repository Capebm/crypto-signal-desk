...
describe('requireLiveConfirmationForStaleLtf', () => {
  ...
  it('builds the Claude paste with T212 ticker and levels', () => {
    ...
    expect(text).toContain('nunca peças')
    expect(text).toContain('Desenhos: BOS')
    expect(text).toContain('ALCANCE')
    expect(text).toContain('pode ser outro trade')
    expect(text).toContain('fecho :00')
    expect(text).toContain('Foto T212 #1 = sempre 5m')
    expect(text).toContain('Foto T212 #2 = sempre 1m')
